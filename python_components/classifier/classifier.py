    ...
    for i, row in tqdm(pdfs.iterrows(), total=len(pdfs), ncols=100):
        ...
        for source in row['source_list']:
            ...
            try:
                ...
                for tag in tags:
                    words_around_link.update([each.lower() for each in re.split('[^a-zA-Z]', tag.get_text()) if len(each) > 0])
...
def get_features(pdfs_path):
    ...
    pdfs['file_name_keywords'] = pdfs['file_name'].apply(lambda x: [chars.lower() for chars in re.split('[^a-zA-Z]', x) if ((len(chars) > 0) and (chars != 'pdf'))])
    pdfs['source_keywords'] = pdfs['source_list'].apply(get_words_from_url_list)
    pdfs['url_keywords'] = pdfs['url'].apply(lambda x: [chars.lower() for chars in re.split('[^a-zA-Z]', urllib.parse.urlparse(x).path) if ((len(chars) > 0) and (chars != 'pdf'))])

    # Convert the human readable file sizes into numerics. TODO: Just export the PDF data with sizes in kilobytes / bytes
    pdfs['file_size_numeric'] = pdfs['file_size'].apply(lambda x: float(x[:-2])*1024 if (x[-2:] == 'MB') else float(x[:-2]))

    # Check for a year in the file name. Could be predictive of an agenda / event
    pdfs['file_name_contains_year'] = pdfs['file_name'].apply(lambda x: 1 if re.search(r'(19|20)\d{2}', x) else 0)
...
def get_feature_matrix(pdfs):
    file_name_dummies = pd.get_dummies(pdfs['file_name_keywords'].explode()).groupby(level=0).sum()
    file_name_dummies.columns = 'file_' + file_name_dummies.columns

    source_dummies = pd.get_dummies(pdfs['source_keywords'].explode()).groupby(level=0).sum()
    ...
    url_text_dummies = pd.get_dummies(pdfs['url_text_keywords'].explode()).groupby(level=0).sum()
    url_text_dummies.columns = 'url_text_' + url_text_dummies.columns

    X = pd.concat([file_name_dummies, source_dummies, url_text_dummies, pdfs[['file_size_numeric', 'number_of_pages', 'file_name_contains_year']]], axis=1)
    return X
...
def get_predictions(feature_matrix, model_path):
    ...
    confidences = [float(probs[category]) for category, probs in zip(predictions, prediction_probs)]
